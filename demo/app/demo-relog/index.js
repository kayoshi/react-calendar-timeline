  ...
  itemRenderer = ({ item }) => {
    return (
      <div className='custom-item'>
        <span className='title'>{item.customId}</span>
        <p className='tip'>{item.itemProps['data-tip']}</p>
      </div>
    )

    // let { item } = itemContainer;
    // let app = this.props.apps[item.id];
    // // console.log('app:', app);

    // let statusType = Utils.safeAccess(() => {
    //   return Statuses.findOne(app.status).type;
    // }, "finished");
    // if (app.late > 0) statusType = "late";
  ...
  render() {
    ...
    return (
      <Timeline
        ...
        sidebarWidth={150}
        sidebarContent={<div>Курьеры</div>}
        canMove={false}
        canResize="right"
        canSelect={false}
